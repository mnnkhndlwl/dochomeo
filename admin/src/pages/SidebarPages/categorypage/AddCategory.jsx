...
function AddCategory({handleClose}) {
  ...
  const [subCategoryField, setSubCategoryField] = useState([{ subcategory: "",subCategoryImage:null}])
...
// ################### MAIN CATEGORY IMAGE UPLOAD TO FIRE BASE ########################
  const handleMainCategoryFileChange =async (e) => {
    ...
    //  const result=await uploadFileToFirebase("/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/",fileUpload.mainCategoryImage)
...
const handleCategoryFileChange =async (e) => {
  ...
  //  const result=await uploadFileToFirebase("/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/",fileUpload.categoryImage)
  // console.log("RESUL AFTER UPLOADED :",result)
};
// ###################  CATEGORY IMAGE UPLOAD TO FIRE BASE ########################
...
const handleSubCategoryFileChange =async(i, e) => {
  ...
  //  const result=await uploadFileToFirebase("/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/",fileUpload)
  // console.log("RESUL AFTER UPLOADED :",result)
};
// ################### SUB CATEGORY IMAGE UPLOAD TO FIRE BASE ########################
 ...
 mainCategoryImageToFirebase = selectMainCategory?.find(value=>value?._id === mainCategory)
  ...
  for(let i=0;i<subCategoryField.length;i++){
    if(subCategoryField[i].subCategoryImage ){
      const getImageData=await uploadFileToFirebase(`/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/allcategories/subcategories/${subCategoryField[i].subcategory}/`,subCategoryField[i].subCategoryImage)
  ...
  await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/create/category`,data,{withCredentials:true})
  .then(response=>{
...
const handleFilterByMainCategory=async(e)=>{
  ...
  return (
      ...
      {/* <img src={fileUpload ? URL.createObjectURL(fileUpload):"" } alt="" />  */}
            ...
            {subCategoryField.map((element,index)=>(
                ...
                : null
                   ...
                   <div className='flex' >
                   <TextField required fullWidth id="outlined-basic" name='subcategory' value={element.subcategory || ""} onChange={e => handleChange(index, e)}  placeholder="Sub Category Name" variant="outlined" />
                     <IconButton color="primary" aria-label="upload picture" component="label">
   ...
   <img className='category-table-image' alt="category-image"   src={subCategoryField[index].subCategoryImage ? URL.createObjectURL(subCategoryField[index].subCategoryImage) :noImage} />
                 ...
                 <Button variant='text' onClick={addFormFields} startIcon={<Iconify icon="ant-design:plus-outlined" />} > Add More </Button>
