...
console.log("ID______________",mainCategoryId)
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/get/category/${mainCategoryId}`,{withCredentials:true})
    .then(res=>{
        console.log(res)
        setData(res?.data)
...
//     deleteImageFromFirebase(`/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/allcategories/maincategories/${main_category_name}/`,image?.image_name)
...
const handleRemoveCategoryImage =async (category_name,image)=>{
    ...
    await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/delete/main/category/image/?old_category_name=${category_name}`,{withCredentials:true})
    .then(res=>{
      console.log(res)
...
const handleRemoveSubCategoryImage=async(i,subcategory_name,image)=>{
  console.log("SUB CATEGORY DELETE IMAGE ==",i,subcategory_name,image)
    ...
    deleteImageFromFirebase(image?.path,image?.image_name)
    await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/delete/main/category/image/?sub_category_id=${mainCategoryId}&old_sub_category_name=${subcategory_name}`,{withCredentials:true})
    .then(res=>{
      console.log(res)
     if(res?.data?.status === true){
      setMessage((prev)=>({...prev,type:"success",message:"Image Deleted Successfully !"}))
...
const handleSubmit = async (e)=>{
  ...
  //    mainCategoryImageToFirebase=await uploadFileToFirebase(`/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/allcategories/maincategories/${mainCategory}/`,fileUpload.mainCategoryImage)

  // }
  if(fileUpload.categoryImage ){
    categoryImageToFirebase=await uploadFileToFirebase(`/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/allcategories/categories/${category}/` ,fileUpload.categoryImage)
  }
  ...
  for(let i=0;i<subCategoryField.length;i++){
    if(subCategoryField[i].subCategoryImage ){
      ...
      const getImageData=await uploadFileToFirebase(`/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/allcategories/subcategories/${subCategoryField[i].name}/`,subCategoryField[i].subCategoryImage)
  ...
  await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/edit/category/${mainCategoryId}`,data,{withCredentials:true})
  .then(response=>{
...
// ##################### SNACK BAR FUNCTIONs ##################
  ...
  return (
   ...
   <img className='category-table-image' alt="category-image"    src={fileUpload.categoryImage ? URL?.createObjectURL(fileUpload?.categoryImage) :allMainAndCategoriesImages?.category_image ? allMainAndCategoriesImages?.category_image : noImage } />
            ...
            {subCategoryField?.map((element,index)=>(
                ...
                index || subCategoryField.length >0 ? 
                  ...
                  <Button variant='text' style={{paddingRight:"46px"}} onClick={() => removeFormFields(index)} > Remove </Button>
                : null
              }
                    </div>
                   <div className='flex' >
   ...
   <img className='category-table-image' alt="category-image"   src={subCategoryField[index]?.subCategoryImage ? URL?.createObjectURL(subCategoryField[index]?.subCategoryImage) :subCategoryField[index]?.image?.image_url ? subCategoryField[index]?.image?.image_url : noImage } />
                 ...
                 <Button variant='text' onClick={addFormFields} startIcon={<Iconify icon="ant-design:plus-outlined" />} > {subCategoryField.length>0 ? "Add More" :"Add Sub Category" } </Button>
