...
function EditProduct({productId,handleClose}) {
   const [productData , setProductData ] = useState({product_main_category:'choose_main_category',product_category:'choose_category',product_subcategory:'choose_sub_category'})
  ...
  const [ openRemoveImageModal , setOpenRemoveImageModal ] =useState([])
   ...
   console.log("PRODUCT ID=>",productId)
       ...
       //================= GET PRODUCT  =================
       useEffect(()=>{
        axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/get/single/product/${productId}`,{withCredentials:true})
        .then(res=>{
          console.log(res);
  ...
  // GET SUB CATEGORY BY BRAND
    ...
    const handleFileUpload = (e)=>{
      ...
      if(productData?.product_images?.length > 4) return alert('4 Product images already exists !! ');
      if(productData?.product_images?.length === 1 && e.target.files?.length >=4 ) return alert('Product image range is 1 - 4');
      if(productData?.product_images?.length === 2 && e.target.files?.length >=3 ) return alert('Product image range is 1 - 4');
      if(productData?.product_images?.length === 3 && e.target.files?.length >=2 ) return alert('Product image range is 1 - 4');
      if(productData?.product_images?.length === 4 && e.target.files?.length >=1 ) return alert('Product image range is 1 - 4');
    ...
    const handleSubmit = async(e)=>{
      ...
      if(fileUpload?.length > 0){
        for(let i = 0; i<fileUpload?.length;i++){
          productsImageToFirebase[i]= await uploadFileToFirebase(`/${process.env.REACT_APP_IMAGES_FOLDER_NAME}/products/${productData?.product_code}/`,fileUpload[i]);
       ...
       let data ={
        ...
        product_subcategory_slug:splitString(productData?.product_subcategory?.toLowerCase()),
...
const handleRemoveProductImage=async(i,image_name,image_path)=>{
  console.log("SUB CATEGORY DELETE IMAGE ==",i,image_name,image_path)
    ...
    await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/remove/product/image/${productId}/${image_name}`,{withCredentials:true})
    .then(res=>{
      console.log(res)
    ...
    {/* <HighlightOffIcon style={{color:palette.primary.main}}  fontSize='large' /> */}
            ...
            <div className='addproduct_img_and_details flex'>
             ...
             <CancelIcon style={{color:'red',cursor:'pointer'}}  onClick={()=>handleOpenRemoveImageModal(index,true)} />
              {/* CONFIRM MODAL */}
           <ConfimModal open={openRemoveImageModal[index]} title="Delete" onYes={()=>handleRemoveProductImage(index,value?.image_name,value?.path)} message="Do you want to delete?" handleClose={()=>handleCloseSubCateConfirmModal(index)}  />
       {/* CONFIRM MODAL */}
                      </div> 
                    </div>
                   ))
             ...
             <CancelIcon style={{color:'red',cursor:'pointer'}}  onClick={()=>handleOpenRemoveImageModal(index,true)} />
              {/* CONFIRM MODAL */}
           <ConfimModal open={openRemoveImageModal[index]} title="Delete" onYes={()=>handleRemoveUploadedImage(index)} message="Do you want to delete?" handleClose={()=>handleCloseSubCateConfirmModal(index)}  />
       {/* CONFIRM MODAL */}
                      </div> 
                    </div>
                   ))
                ...
                <div className='add_product_form'>
                  ...
                  <div className='add_product_label_input' style={{width:'100%'}} >
                    ...
                    <TextField multiline rows={10}  fullWidth className='product_form_input' name='product_description' value={productData?.product_description} onChange={handleChange} id="outlined-basic" placeholder=" Add Description " variant="outlined" />
                    </div>
