...
import AddCoupons from "./SidebarPages/couponpage/AddCoupons";
import EditCoupon from "./SidebarPages/couponpage/EditCoupons";
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component="div"
        >
          {`All Coupons(${props.productCount})`}
        </Typography>
      )}
            ...
            <Tooltip title="">
              <IconButton>
                ...
                >
                  ...
                  <MenuItem
                    sx={{ color: "text.secondary" }}
                    onClick={() => props.setAsTrendingProducts(props.selected)}
                  >
                    <ListItemIcon>
                      <Iconify
                    ...
                    <ListItemText
                      primary="Set As Trending Product"
                      primaryTypographyProps={{ variant: "body2" }}
                    />
                  </MenuItem>
                  <MenuItem
                    sx={{ color: "text.secondary" }}
                    onClick={() =>
                      props.removeAsTrendingProducts(props.selected)
                    }
                  >
                    <ListItemIcon>
                      <Iconify
...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/api/coupon/all`, {
        withCredentials: true,
      })
      .then((res) => {
        console.log(res);
        setData(res?.data?.coupons);
        setProductCount(res?.data?.coupons?.length);
  ...
  const handleSearch = async (e) => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/coupon/search?search=${searchValue}`,
        { withCredentials: true }
      )
      .then((res) => {
        console.log(res);
  ...
  const setAsTrendingProducts = async (value) => {
    await axios
      .patch(
        `${process.env.REACT_APP_BACKEND_URL}/api/set/products/as/trending/products`,
        { data: value },
        { withCredentials: true }
      )
      .then((res) => {
        ...
        setMessage((prev) => ({
          ...prev,
          type: "success",
          message: "Product Added to Trending Products Successfully !",
        }));
        setSnackbarOpen(true);
      })
      .catch((err) => {
        ...
        setMessage((prev) => ({
          ...prev,
          type: "success",
          message: "Product Added to Trending Products Failed !",
        }));
        setSnackbarOpen(true);
      });
  };
  ...
  const removeAsTrendingProducts = async (value) => {
    await axios
      .patch(
        `${process.env.REACT_APP_BACKEND_URL}/api/remove/products/as/trending/products`,
        { data: value },
        { withCredentials: true }
      )
      .then((res) => {
        ...
        setMessage((prev) => ({
          ...prev,
          type: "success",
          message: "Product Removed From Trending Products Successfully !",
        }));
        setSnackbarOpen(true);
      })
      .catch((err) => {
