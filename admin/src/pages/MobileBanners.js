...
export default function MobileBanners() {
  ...
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/mobile/get/all/banners`,{withCredentials:true})
    .then(res=>{
...
const handleRemoveBannerImage=async(i,bannerId,image_name,image_path)=>{
    console.log(" DELETE BANNER ==",bannerId,image_name,image_path)
      ...
      await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/mobile/delete/banner/by/id/${bannerId}`,{withCredentials:true})
      .then(res=>{
        console.log(res)
...
//    console.log("IMAGES AFTER FIREBASE",bannersToFirebase);
    if(bannersToFirebase.image_url){
     await axios.post(`${process.env.REACT_APP_BACKEND_URL}/api/mobile/add/new/banner`,{...bannersToFirebase},{withCredential:true})
        .then(res=>{
            console.log(res?.data);
...
//    console.log("IMAGES AFTER FIREBASE",bannersToFirebase);
    if(bannersToFirebase.image_url){
     await axios.patch(`${process.env.REACT_APP_BACKEND_URL}/api/mobile/change/banner/by/id/${bannerId}`,{...bannersToFirebase},{withCredential:true})
        .then(res=>{
            console.log(res?.data);
            if(res?.data?.status){
          ...
          >
           ...
           <ConfimModal open={openRemoveImageModal[index]} title="Delete" onYes={()=>handleRemoveBannerImage(index,row?._id,row?.image_name,row?.path)} message="Do you want to delete?" handleClose={()=>handleCloseSubCateConfirmModal(index)}  />
                    ...
                    >
                      ...
                      <TableCell style={{textTransform:'capitalize'}} align="left">{returnFileName(row?.image_name)}</TableCell>

                      <TableCell align="center">{convertDate(row?.updatedAt)}</TableCell>
               ...
               <Button onClick={()=>handleOpenRemoveImageModal(index,true)} variant="text" style={{color:'red',marginLeft:8,boxShadow:'none'}}  startIcon={<Iconify icon="eva:trash-2-outline" />}> 
           Remove
               </Button>
                          </TableCell>
                    </TableRow></>
