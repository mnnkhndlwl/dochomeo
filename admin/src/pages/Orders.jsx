...
import { Toolbar,TextField, Tooltip,Menu, MenuItem, IconButton, Typography,Button,ListItemIcon, ListItemText, OutlinedInput, InputAdornment } from '@mui/material';
...
import { convertDateForOrder ,getGapBetweenDates} from '../global/globalFunctions';
...
export default function EnhancedTable() {
  ...
  useEffect(()=>{
    setLoading(true)
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/get/all/orders`,{withCredentials:true})
    .then(res=>{
      console.log(res)
...
const handleDleteOrders = async(value)=>{
  ...
  await axios.delete(`${process.env.REACT_APP_BACKEND_URL}/api/delete/order/by/id`,{data: value},{withCredentials:true})
  .then(res=>{
    console.log(res)
  ...
  return (
    ...
    <div className='custom-conatiner'>
     ...
     <SideDrawer state={drawerEditOrders} toggleDrawerClose={handleCloseEditOrderSideBar} toggleDrawerOpen={handleOpenEditOrderSidebar}
           ComponentData={<ViewOrder handleClose={handleCloseEditOrderSideBar} orderId={orderIdForEdit} />}
            />
                 {/*################ EDIT PRODUCT SIDEBAR  ################*/}
                   {/* CONFIRM MODAL */}
          <ConfimModal open={openDeleteConfimModal} title="Delete" onYes={()=>handleDleteOrders(selected)} message="Do you want to delete?" handleClose={handleCloseConfimModal}  />
       {/* CONFIRM MODAL */}
    <Box sx={{ width: '100%' }}>
   ...
   <TextField id="outlined-basic" type="search" value={search} onChange={(e)=>setSearch(e.target.value)} fullWidth  label="Search In Orders" placeholder='Search Anything...' variant="outlined"  InputProps={{
          startAdornment: (
   ...
   <Button sx={{mx:2,height:54,px:5}} variant="contained" type='submit'>
        Search
      </Button>
   </form>
...
<Paper elevation={3} sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar setOpenDeleteConfimModal={setOpenDeleteConfimModal}  orderCount={ordersCount} numSelected={selected.length} />
        <TableContainer>
          <Table
            id="order-to-xls"
            sx={{ minWidth: 750 }}
                ...
                ?.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="center" style={{textTransform:'capitalize'}} >{row.customer_name}</TableCell>
                      {/* <TableCell align="right">{row.fat}</TableCell> */}
                      <TableCell align="center">+91-{row.customer_phone_number}</TableCell>
                      {/* <TableCell align="right">{row.protein}</TableCell> */}
                      <TableCell align="center"> <p  >{row.products?.length}</p> </TableCell>
                      <TableCell align="left"> 
                      <p style={{color:palette.primary.main,fontWeight:'600'}} > ₹ 199</p>
                        </TableCell>
                     
                      <TableCell align="center">
                        <p className={row.order_status === "delivered" ? "order_delivered" :
                          row.order_status === "partial shipped" ? "order_delivered"
                         : row.order_status === "cancelled"? "order_cancel"
                         : row.order_status === "shipped" ?"order_processing"
                         : row.order_status === "pending"? "order_pending"
                         : "normal"  } >
      ...
      >
        ...
        <MenuItem value={30}>Cancel</MenuItem>
                ...
                {!allOrders?.length >0 &&   <TableCell colSpan={9}> <div className='search-not-found' >
