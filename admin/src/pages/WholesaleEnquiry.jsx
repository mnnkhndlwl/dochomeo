...
import WholesaleEnquiryPreview from "src/global/Modals/WholesaleEnquiryPreview";
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component="div"
        >
          {`All Wholesale Enquiries (${props.orderCount})`}
        </Typography>
      )}
...
export default function EnhancedTable() {
  ...
  const [openWholesaleEnquiryPreview, setOpenWholesaleEnquiryPreview] =
    useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [isOpen2, setIsOpen2] = useState(false);
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/api/wholesaleEnquires`, {
  ...
  // SEARCH IN wholesale table
  const handleSearch = async (e) => {
    e.preventDefault();
    setLoading(true);
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/search/wholesale?search=${search}`,
        { withCredentials: true }
      )
      .then((res) => {
        setAllEnquiries();
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = allEnquiries?.map((n) => n._id);
  ...
  function handleCloseWholesaleEnquiryPreview() {
    setOpenWholesaleEnquiryPreview(false);
  }

  return (
    <>
      <LoadingSpinner loading={loading} />
      <WholesaleEnquiryPreview
        data = {previewData}
        open={openWholesaleEnquiryPreview}
        handleClose={handleCloseWholesaleEnquiryPreview}
      />
        ...
        <Box sx={{ width: "100%" }}>
          <Paper elevation={3} sx={{ width: "100%", mb: 2, borderRadius: 1 }}>
            <div className="product-topbar-box vendor-topbar-box ">
              <h3 className="">Wholesale Enquiries</h3>
            ...
            <TableContainer>
              ...
              >
                <EnhancedTableHead
                  ...
                  rowCount={allEnquiries?.length}
                />
                <TableBody>
                  {/* if you don't need to support IE11, you can replace the `stableSort` call with:
                 rows.slice().sort(getComparator(order, orderBy)) */}
                  {stableSort(allEnquiries, getComparator(order, orderBy))
                    ?.slice(
                      page * rowsPerPage,
        ...
        <MenuItem value={30}>Cancel</MenuItem>
                          ...
                          <TableCell align="center">
                            ...
                            <VisibilityIcon
                              ...
                              fontSize="small"
                              onClick={() => {
                                setOpenWholesaleEnquiryPreview(true);
                                setPreviewData(row);
