...
import AddDoctor from "./SidebarPages/doctorpage/AddDoctor";
import EditDoctor from "./SidebarPages/doctorpage/EditDoctor";
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography
          ...
          component="div"
        >
          {`All Available Doctors (${props.orderCount})`}
        </Typography>
      )}
...
export default function EnhancedTable() {
  ...
  const [drawerEditDoctor, setDrawerEditDoctor] = React.useState(false);
  const [drawerAddDoctor, setDrawerAddDoctor] = React.useState(false);
  const [isOpen, setIsOpen] = useState(false);
  const [isOpen2, setIsOpen2] = useState(false);
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${process.env.REACT_APP_BACKEND_URL}/api/get/all/doctors/`, {
  ...
  const handleSearch = async (e) => {
    ...
    await axios
      .get(
        `${process.env.REACT_APP_BACKEND_URL}/api/search/in/doctors?search=${search}`,
        { withCredentials: true }
      )
      .then((res) => {
        setAllDoctors();
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = allDcotors?.map((n) => n._id);
  ...
  const handleOpenEditDoctor = () => {
    setDrawerEditDoctor(true);
  };

  const handleCloseEditDoctor = () => {
    setDrawerEditDoctor(false);
    setRender((prev) => !prev);
        ...
        <Box sx={{ width: "100%" }}>
          <Paper elevation={3} sx={{ width: "100%", mb: 2, borderRadius: 1 }}>
            <div className="product-topbar-box vendor-topbar-box ">
              <h3 className="">Available Doctors</h3>
              <div className="product-topbar-btn">
                ...
                <SideDrawer
                  ...
                  ComponentData={
                    <AddDoctor handleClose={handleCloseAddProductSideBar} />
                  }
                  ComponentButton={
                    <Button
                ...
                />
                {/* <Button  className='product-btn' variant="contained" startIcon={<Iconify icon="eva:plus-fill" />}> 
          Add New Doctor
         </Button> */}
                {/*################ ADD PRODUCTS SIDEBAR BUTTON ###############*/}
            ...
            <TableContainer>
              ...
              >
                <EnhancedTableHead
                  ...
                  rowCount={allDcotors?.length}
                />
                <TableBody>
                  {/* if you don't need to support IE11, you can replace the `stableSort` call with:
                 rows.slice().sort(getComparator(order, orderBy)) */}
                  {stableSort(allDcotors, getComparator(order, orderBy))
                    ?.slice(
                      page * rowsPerPage,
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell align="center">
                            ...
                            {row.time?.startTime} AM - {row.time.endTime} PM
                          </TableCell>
