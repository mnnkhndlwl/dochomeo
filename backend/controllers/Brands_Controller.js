...
const createCategory = async (req, res) => {
  ...
  try {
    const findExistingCategory = await Brands_Schema.find({
    ...
    if (findMainCategory[0]?.category_name) {
      ...
      const updateCategory = await Brands_Schema.findByIdAndUpdate(
...
const updateMainCategory = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      const find = await Brands_Schema.find({
        main_category_name: oldMainCategory,
      });
      console.log("FIND___", find);
      if (find?.length > 0) {
        const updateAll = await Brands_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
...
const deleteImage = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      ...
      if (find?.length > 0) {
        const removeImage = await Brands_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
              main_category_image: {},
    ...
    if (oldCategory) {
      const findCategory = await Brands_Schema.find({
    ...
    if (oldSubId && oldSubCategory) {
      ...
      const findSubCategroy = await Brands_Schema.find({
        _id: oldSubId,
      ...
      if (findSubCategroy?.length > 0) {
        ...
        const updateSubCategory = await Brands_Schema.findByIdAndUpdate(
          { _id: oldSubId },
...
const searchInCategory = async (req, res) => {
  ...
  try {
    const countCategory = await Brands_Schema.find({}).count();
    result = await Brands_Schema.find({
